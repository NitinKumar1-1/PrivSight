...
export async function renderMaskedPreview(capture: Capture, regions: BBox[]): Promise<string | null> {
  const response = await offscreen({ target: "offscreen", type: "MASK_IMAGE", dataUrl: capture.dataUrl, regions });
  if (!response.ok) throw new Error(response.error);
